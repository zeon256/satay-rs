...
- Rust structs, flattened `allOf` object structs, ref-only `anyOf`/`oneOf` unions, supported discriminator unions, string enums, primitive aliases, and constrained newtypes from `components.schemas`.
