# Reqwest Blocking Example

This example generates code from `../openapi.yaml` at build time, builds a Sans-IO request with Satay's generated action API, sends it with `reqwest::blocking`, and decodes the response with the generated response enum.

The example imports `reqwest` through `satay-reqwest`'s re-export, while keeping a direct `reqwest` dependency in `Cargo.toml` to choose the blocking and TLS features.
