...
## [0.17.5](https://github.com/zeon256/satay-rs/compare/satay-codegen-v0.17.3...satay-codegen-v0.17.5) - 2026-09-19
...
- *(codegen)* switch generation to semantic IR
...
- *(codegen)* ensure that we dont emit fully qualified path

## [0.17.2](https://github.com/zeon256/satay-rs/compare/satay-codegen-v0.17.1...satay-codegen-v0.17.2) - 2026-09-12
...
- *(codegen)* apply parameter defaults in request builders

### Added

- *(codegen)* apply supported scalar OpenAPI schema defaults to optional query and header parameters
...
- *(codegen)* reject property-only `x-satay` keys on value schemas by presence, including explicit `false`, while retaining them beside object-property `$ref` schemas
- *(codegen)* reject every present but inapplicable schema `x-satay` key across component structs, enums, and `$ref` siblings instead of silently discarding it
- *(codegen)* reject `x-satay.ignore: true` combined with any other schema-level `x-satay` key; `ignore: false` remains combinable

## [0.16.1](https://github.com/zeon256/satay-rs/compare/satay-codegen-v0.16.0...satay-codegen-v0.16.1) - 2026-08-18

### Fixed

- *(codegen)* shorten runtime serde module paths in
- *(codegen)* import runtime serde modules in generated types
