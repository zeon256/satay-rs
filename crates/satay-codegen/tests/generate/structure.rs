...
fn simple_fixture_generates_expected_file_structure() {
    ...
    let users_source = find_file(&files, "users.rs");
    assert!(users_source.contents.starts_with("// @generated by satay."));
    assert!(!users_source.contents.contains("//! @generated by satay."));
    ...
    assert!(is_pub(&find_mod(&mod_rs, "operations").vis));
    let users_mod = find_mod(&mod_rs, "users");
    assert!(is_pub(&users_mod.vis));
    assert!(has_cfg_feature(&users_mod.attrs, "json"));
    ...
    find_method(&api_rs, "Api", "users");
    assert!(!has_method(&api_rs, "Api", "get_user"));
    find_method(&api_rs, "GetUserAction", "request");
    find_method(&api_rs, "GetUserAction", "decode");

    let users_rs = parse_rust(find_file(&files, "users.rs"));
    find_struct(&users_rs, "Api");
    find_method(&users_rs, "Api", "get_user");
    find_method(&users_rs, "Api", "update_user");
...
#[test]
fn operation_tags_generate_namespaced_api_groups() {
    let files = codegen::generate(GROUPED).expect("generate grouped fixture");

    let root = parse_rust(find_file(&files, "mod.rs"));
    for group in ["realtime", "bus", "untagged"] {
        let module = find_mod(&root, group);
        assert!(is_pub(&module.vis));
        assert!(has_cfg_feature(&module.attrs, "json"));
    }
    assert_doc(&find_mod(&root, "bus").attrs, "Bus operations.");
    ...
    find_method(&realtime, "Api", "get_bus_arrival");

    let untagged = parse_rust(find_file(&files, "untagged.rs"));
    find_method(&untagged, "Api", "health");
}

#[test]
fn lib_root_module_option_emits_lib_rs_instead_of_mod_rs() {
    use super::codegen::{GenerateOptions, RootModule};
    let options = GenerateOptions {
        root_module: RootModule::LibRs,
    };
    let files = codegen::generate_with(SIMPLE, options).expect("generate simple fixture");
...
"#,
    ...
    let api_rs = parse_rust(find_file(&files, "untagged.rs"));
    assert_doc(
...
"#,
    ...
    find_method(&api_rs, "Api", "untagged");
    find_struct(&api_rs, "GetUserAction");
...
fn petstore_minimal_fixture_generates_client_core() {
    let files = codegen::generate(PETSTORE_MINIMAL).expect("generate petstore fixture");
