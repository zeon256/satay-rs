...
pub(super) fn excluded_component_schemas(
    ...
    if let Some(paths) = document.spec.paths.as_ref() {
        for (path, path_item) in paths {
            ...
            let params_scope = if path_params_kept {
                MediaScope::Json
            } else {
                MediaScope::All
            };
            for parameter in &path_item.parameters {
                collect_parameter_schema_refs(document, parameter, params_scope, params_bucket)?;
            }
...
/// from `roots`. Cycle-safe via `seen`.
fn closure(document: &ResolvedDocument<'_>, roots: BTreeSet<String>) -> BTreeSet<String> {
    let mut seen = BTreeSet::new();
    let mut worklist = roots.into_iter().collect::<Vec<String>>();
...
/// Collects a parameter's schema and content references.
fn collect_parameter_schema_refs<'a>(
    ...
    if let Some(content) = parameter.content.as_ref() {
        collect_content_schema_refs(content, scope, out);
    }
    Ok(())
}
...
fn collect_operation_schema_refs<'a>(
    ...
    for parameter in &operation.parameters {
        collect_parameter_schema_refs(document, parameter, scope, out)?;
    }
