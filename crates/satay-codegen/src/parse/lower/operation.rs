...
use super::schema::parse_type_ref_with_hint;
use crate::error::ValidationError;
use crate::ident::{field_ident, function_ident, response_variant_ident, type_ident, unique_ident};
use crate::model::{
    ApiKeyLocation, ApiKeySecurityScheme, Operation as SatayOperation, Parameter, RequestBody,
    ResponseCase,
};
...
pub(super) fn parse_operations(
    ...
    document
        .operations
        .iter()
        .map(|operation| parse_operation(operation, registry))
        .collect()
}

fn parse_operation(operation: &ValidatedOperation, registry: &mut TypeRegistry) -> SatayOperation {
    ...
    let mut parameters = operation
        .parameters
        .iter()
        .map(|parameter| parse_parameter(parameter, registry, &type_prefix))
        .collect::<Vec<_>>();
    ...
    let responses = operation
        .responses
        .iter()
        .map(|response| parse_response(response, registry, &type_prefix))
        .collect();
