...
fn render_top_mod(api: &Api) -> syn::File {
    ...
    for group in &api.groups {
        let module = ident(&group.rust_name);
        let docs = doc_attrs(group.description.as_deref());
        items.push(parse_quote!(
            #(#docs)*
            #[cfg(feature = "json")]
            pub mod #module;
    ...
    for operation in &api.operations {
        ...
        items.push(parse_quote!(mod #module;));
        items.push(parse_quote!(pub use #module::{#input, #response};));
    }
...
fn render_operations_mod(api: &Api) -> syn::ItemMod {
    let operation_modules = api
        .operations
        .iter()
