...
pub mod serde_string {
    ...
    pub mod as_bool {
        ...
        impl Visitor<'_> for MappedBoolNoneIfVisitor<'_> {
            ...
            fn expecting(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
                formatter.write_str(
                    "a configured boolean string, numeric boolean, or configured sentinel",
                )
            }
        ...
        pub mod option {
                ...
                S: serde::Serializer,
            {
                match value {
                    Some(value) => {
                        super::serialize_mapped(value, true_value, false_value, serializer)
                    }
                    None => serializer.serialize_none(),
                }
            }
            ...
            impl<'de> Visitor<'de> for MappedBoolOptionVisitor<'_> {
                    ...
                    D: serde::Deserializer<'de>,
                {
                    match self.none_if {
                        Some(none_if) => {
                            deserializer.deserialize_any(super::MappedBoolNoneIfVisitor {
    ...
    pub mod pair {
            ...
            D: serde::Deserializer<'de>,
        {
            deserializer.deserialize_str(StringVisitor(|value: &str| -> Result<_, D::Error> {
                super::parse_none_if_with(value, none_if, |value| {
                    parse_with(value, delimiter, parse)
                })
            }))
        }
        ...
        pub mod option {
                ...
                D: serde::Deserializer<'de>,
            {
                deserializer.deserialize_option(OptionVisitor(
                    |value: &str| -> Result<_, D::Error> {
                        parse_none_if_with(value, none_if, |value| {
                            parse_with(value, delimiter, parse)
                        })
                    },
                ))
