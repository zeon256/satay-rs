...
use log::trace;
use oas3::{
    spec::{BooleanSchema, Error as SchemaError, ObjectSchema, Schema, SchemaType, SchemaTypeSet},
    Spec,
};
...
impl ValidationTree {
    ...
    pub(crate) fn from_schema_document(
        ...
        match schema.resolve(spec).unwrap() {
            ...
            Schema::Object(sub_schema) => ValidationTree::from_schema(&sub_schema, spec),
        }
    }
