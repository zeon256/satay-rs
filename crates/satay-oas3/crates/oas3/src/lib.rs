...
//! - `yaml-spec`: Enables YAML parsing and serialization with `serde-saphyr`.
//!
...
/// Error returned while deserializing an OpenAPI YAML document.
#[cfg(feature = "yaml-spec")]
pub use serde_saphyr::DeserializeError as YamlDeserializeError;
/// Error returned while serializing an OpenAPI YAML document.
#[cfg(feature = "yaml-spec")]
pub use serde_saphyr::SerializeError as YamlSerializeError;
...
) -> std::io::Result<Result<OpenApiV3Spec, YamlDeserializeError>> {
...
#[cfg(all(test, feature = "yaml-spec"))]
pub(crate) fn from_reader(read: impl std::io::Read) -> Result<OpenApiV3Spec, YamlDeserializeError> {
    serde_saphyr::from_reader_with_options(read, yaml_options())
}

/// Deserializes an OpenAPI spec (YAML-format) from a string.
#[cfg(feature = "yaml-spec")]
pub fn from_yaml(yaml: impl AsRef<str>) -> Result<OpenApiV3Spec, YamlDeserializeError> {
    serde_saphyr::from_str_with_options(yaml.as_ref(), yaml_options())
}
...
#[cfg(feature = "yaml-spec")]
pub fn to_yaml(spec: &OpenApiV3Spec) -> Result<String, YamlSerializeError> {
    serde_saphyr::to_string(spec)
}
...
mod tests {
    ...
    fn compare_spec_through_json(
        ...
        // First conversion:
        //     File -> `String` -> `serde_json::Value` -> `String`
