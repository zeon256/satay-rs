...
use super::{discriminator::Discriminator, spec_extensions, FromRef, Ref, RefError, RefType, Spec};
use crate::Map;
...
pub struct ObjectSchema {
    /// URI reference identifying another schema.
    ///
    /// Unlike an OpenAPI Reference Object, `$ref` is a JSON Schema keyword in a
    /// Schema Object. Other schema keywords and specification extensions beside
    /// it remain part of this object and are therefore preserved.
...
impl Schema {
    ...
    ///
    /// This is a navigation helper, not a JSON Schema evaluator. Keywords and extensions beside
    /// `$ref` remain available on the original [`ObjectSchema`] and are not merged into the
    /// returned target.
    pub fn resolve(&self, spec: &Spec) -> Result<Self, RefError> {
        match self {
            Schema::Boolean(schema) => Ok(Schema::Boolean(schema.clone())),

            Schema::Object(schema) => match schema.reference.as_deref() {
                Some(reference) => Schema::from_ref(spec, reference),
...
mod tests {
    ...
    fn type_set_contains() {
        ...
        let schema = serde_saphyr::from_str::<ObjectSchema>(spec).unwrap();
    ...
    fn prefix_items_supports_inline_and_reference_schemas() {
        ...
        assert_matches!(
            ...
            Schema::Object(obj) if matches!(
                obj.as_ref(),
                ObjectSchema { reference: Some(ref_path), .. }
                    if ref_path == "#/components/schemas/Age"
            ),
    ...
    #[test]
    fn schema_reference_preserves_keywords_and_extensions() {
        let schema = serde_saphyr::from_str::<Schema>(indoc::indoc! {"
            $ref: '#/components/schemas/BusArrivalTiming'
            description: Next available bus.
            x-satay:
        ...
        assert_eq!(
            ...
            Some("#/components/schemas/BusArrivalTiming")
        );
        assert_eq!(schema.description.as_deref(), Some("Next available bus."));
        assert_eq!(
            schema.extensions["satay"]["treat-error-as-none"],
            serde_json::Value::Bool(true)
        );

        let serialized = serde_json::to_value(schema).unwrap();
        assert_eq!(serialized["$ref"], "#/components/schemas/BusArrivalTiming");
        assert_eq!(
            serialized["x-satay"]["treat-error-as-none"],
            serde_json::Value::Bool(true)
        );
    }
    ...
    fn prefix_items_serialization_round_trip() {
        ...
        let original = serde_saphyr::from_str::<ObjectSchema>(spec).unwrap();
