...
Satay's lossless Schema Object fork of `oas3`. The Rust library name remains
`oas3` so existing imports continue to work.

> This directory is Satay's source fork of `x52dev/oas3-rs`. It retains the
> upstream integration tests and fixtures as members of Satay's root Cargo
> workspace; see [`UPSTREAM.md`](UPSTREAM.md) for the pinned release and commit.
