...
- `anyOf` and `oneOf` unions whose branches are local `#/components/schemas/...` references or inline single-value string enums, rendered as ordered serde-untagged Rust enums. Plain `oneOf` support does not enforce JSON Schema's exact-one validation rule.
