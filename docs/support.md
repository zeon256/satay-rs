...
- String enums are generated as strict Rust enums. Open string enum shapes such as `anyOf: [{type: string}, {type: string, enum: [...]}]` generate known variants plus `Other(String)` so unknown values round-trip. `Other` is reserved only for that open-enum fallback; closed enums may use `Other` as a normal declared variant name.
- `anyOf` and `oneOf` unions whose branches are local `#/components/schemas/...` references, inline string enums, inline primitive schemas (`string`, `integer`, `number`, `boolean`, or `array` with supported `items`), or a single inline `type: "null"` branch. Null branches make the union type nullable and are rendered as `Option<Union>`, not as enum variants. Plain unions are rendered as ordered serde-untagged Rust enums, and plain `oneOf` support does not enforce JSON Schema's exact-one validation rule.
- `anyOf` or `oneOf` unions with an OpenAPI `discriminator`, when every branch is a local `#/components/schemas/...` reference to an object struct component. If branch structs do not contain the discriminator property, these render as serde internally tagged Rust enums. If every branch contains the discriminator property as a required singleton string enum, these render as serde untagged Rust enums and keep the branch field. Unmapped internally tagged branches use their component schema name as the wire tag value; explicit `mapping` entries may override individual branches and may target local `#/components/schemas/...` refs or bare component schema names.
