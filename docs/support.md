...
- Satay-specific operation-level `x-satay.output` response projection, selecting a top-level `unwrap-field` and optionally mapping one field from every item of an unwrapped array.
...
- OpenAPI `1XX`–`5XX` wildcard response keys, decoded as range variants (`Informational`, `Success`, `Redirection`, `ClientError`, `ServerError`) carrying the concrete `http::StatusCode` plus the JSON body when one is declared; explicit status codes take precedence over the range covering them.
...
- Field-local `x-satay.parse-as: coordinates` codecs that reuse a referenced generated two-number object, with explicit field order, a configurable literal delimiter, existing numeric validation, and optional/sentinel/lossy decoding. See [coordinate codecs](extensions.md#parse-as-coordinates) for placement and serialization rules.
- Satay-specific `x-satay.true-values`, `x-satay.false-values`, and `x-satay.unknown-as` options for exact, canonical string-backed boolean mappings on struct fields and parameters.
- Satay-specific `x-satay.none-if` sentinel lists for string-backed parsed struct fields, producing `Option<T>` while preserving errors for unconfigured invalid strings.
...
- Satay-specific `x-satay.identifier` overrides for target-neutral public property names, using canonical lower-kebab-case words that the Rust backend renders as snake_case while preserving the original wire key.
...
- Nullable required parameters and nullable path parameters. On optional query/header parameters, nullability (`anyOf`/`oneOf` with a `type: "null"` branch, or `type: [T, "null"]`) folds into optionality and renders `Option<T>`.
